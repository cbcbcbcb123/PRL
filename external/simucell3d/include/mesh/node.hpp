...
class node final 
{
    ...
    public:
        ...
        node() noexcept;
        node(const node& n) noexcept;            //copy constructor
        node(node&& n) noexcept;                 //move constructor
        node& operator=(const node& n) noexcept; //copy assignment operator
        node& operator=(node&& n) noexcept;      //move assignment operator
        ...
        // OpenMP locks are process resources and cannot be bitwise copied.
        // Every constructor owns a freshly initialized lock; assignments keep
        // the destination lock and copy only the mechanical state.
        ~node();
