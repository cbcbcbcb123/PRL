...
std::vector<oriented_point> poisson_sampling::uniform_sampling(const cell_ptr input_cell, const double l_min) noexcept(false){
    ...
    // MSVC's OpenMP front-end does not support this user-defined vector
    // reduction.  The original loop also shares one random-number engine
    // between threads.  Use the same serial semantics on MSVC; other
    // toolchains retain the upstream parallel implementation.
    #ifndef _MSC_VER
