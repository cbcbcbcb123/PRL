...
void ball_pivoting_algorithm::find_seed_triangle() noexcept(false){
    ...
    // std::random_shuffle was removed in C++17.  Keep the PRL fork
    // deterministic so a seed search is reproducible across platforms.
    std::mt19937 generator(0x53433344U);
    std::shuffle(index_lst.begin(), index_lst.end(), generator);
