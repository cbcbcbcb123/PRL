...
contact_node_face_via_spring::run_surface_quadrature(
    const std::vector<cell_ptr>& cells, double maximum_edge, double adhesion_scale, double positive_barrier_range) const {
    ...
    struct target_face {face* f; std::array<vec3,3> p; std::array<double,6> box;};
    ...
    for(const auto& source:cells) for(const auto& target:cells) {
        if(source.get()==target.get())continue;
        std::vector<target_face> target_faces;
        ...
        std::vector<vec3> vertex_normals(target->node_lst_.size(),vec3(0,0,0));
        for(auto& f:target->face_lst_)if(f.is_used()){
            ...
            target_faces.push_back({&f,p,bounds(p)});
            for(unsigned i=0;i<3;++i){
                const unsigned j=(i+1)%3,k=(i+2)%3;
                const auto key=std::minmax(ids[i],ids[j]);
                auto iter=edge_normals.find(key);
                if(iter==edge_normals.end())edge_normals.emplace(key,f.normal_);
                ...
                vertex_normals[ids[i]]=vertex_normals[ids[i]]+f.normal_*std::acos(cosine);
            }
        }
        for(auto& source_face:source->face_lst_)if(source_face.is_used()){
            const auto ids=source_face.get_node_ids();
            std::array<vec3,3> p={source->node_lst_[ids[0]].pos(),source->node_lst_[ids[1]].pos(),source->node_lst_[ids[2]].pos()};
            const auto box=bounds(p);std::vector<const target_face*> candidates;
            for(const auto& f:target_faces){
                bool nearby=true;
                for(unsigned axis=0;axis<3;++axis)
                    nearby=nearby && box[axis]<=f.box[axis+3]+cutoff && box[axis+3]>=f.box[axis]-cutoff;
                if(nearby)candidates.push_back(&f);
            }
            ...
            const std::array<vec3,3> area_gradient={(p[1]-p[2]).cross(normal)*.5,(p[2]-p[0]).cross(normal)*.5,(p[0]-p[1]).cross(normal)*.5};
            ...
            while(edge>maximum_edge){
                ...
                for(const auto& t:patches){
                    const auto ab=(t[0]+t[1])*.5,bc=(t[1]+t[2])*.5,ca=(t[2]+t[0])*.5;
                    refined.push_back({t[0],ab,ca});refined.push_back({ab,t[1],bc});
                    refined.push_back({ca,bc,t[2]});refined.push_back({ab,bc,ca});
                }patches=std::move(refined);edge*=.5;
            }
            const double fraction=0.5/(3.0*patches.size()); // two directions, three samples per patch
            const double weight=0.5*twice_area*fraction;
            for(const auto& patch:patches)for(unsigned sample=0;sample<3;++sample){
                ...
                const vec3 q=p[0]*sb[0]+p[1]*sb[1]+p[2]*sb[2];
                const auto qc=coordinates(q);const target_face* best=nullptr;
                double distance_square=cutoff*cutoff;vec3 target_bary;
                for(const auto* f:candidates){
                    double box_distance=0;
                    for(unsigned axis=0;axis<3;++axis){
                        const double delta=std::max({f->box[axis]-qc[axis],qc[axis]-f->box[axis+3],0.0});box_distance+=delta*delta;
                    }
                    if(box_distance>distance_square)continue;
                    const auto result=compute_node_triangle_distance(q,f->p[0],f->p[1],f->p[2]);
                    // Strict update gives stable unique ownership at shared edges.
                    if(result.first<distance_square){distance_square=result.first;target_bary=result.second;best=f;}
                }
                if(!best)continue;
                ...
                const auto target_ids=best->f->get_node_ids();
                std::vector<unsigned> feature;
                for(unsigned i=0;i<3;++i)if(tb[i]>1e-9)feature.push_back(target_ids[i]);
                vec3 sign_normal=best->f->normal_;
                if(feature.size()==1)sign_normal=vertex_normals[feature[0]];
                if(feature.size()==2)sign_normal=edge_normals.at(std::minmax(feature[0],feature[1]));
                const double sign=delta.dot(sign_normal)>=0.0?1.0:-1.0;
                const double gap=sign*distance;
                const auto& material=target->get_face_type(best->f->local_face_id_);
                const double ka=material.adherence_strength_*adhesion_scale,kr=material.repulsion_strength_;
                ...
                if(gap<0.0){
                    if(distance>=interaction_cutoff_repulsion_)continue;
                ...
                else if(positive_barrier_range==0.0 || gap>=positive_barrier_range)continue;
