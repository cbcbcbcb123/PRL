...
/**
 * Stable observer seam between a cell engine and PRL material-state ownership.
 *
 * A SimuCell3D adapter must emit one callback after every accepted split, swap,
 * or merge. Snapshots use PRL-owned value types; no upstream pointer or face ID
 * may cross this boundary. Implementations must call observers synchronously,
 * before a later topology edit can invalidate the after snapshot.
 */
