...
def test_v04_crosses_the_frozen_step_86_with_all_physical_gates():
    ...
    assert trajectory.time[-1] == pytest.approx(1.72, abs=1.0e-15)
    assert len(trajectory.time) == 87
    assert np.all(np.isfinite(trajectory.vertices))

    audit = run.step_audits[-1]
    assert audit.step_index == 86
