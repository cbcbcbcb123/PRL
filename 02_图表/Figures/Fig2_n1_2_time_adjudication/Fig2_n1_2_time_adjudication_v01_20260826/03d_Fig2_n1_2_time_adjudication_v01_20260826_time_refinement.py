...
SUPPORTED_TIME_LEVELS = (16, 32, 64)
...
def resample_uniform_cycle_history(
    ...
    *,
    ...
    between the registered 16/32/64 grids is allowed, and all source grid points
...
def dense_cycle_peak(
    ...
    *,
    ...
    if mode == "max":
        index = int(np.argmax(dense_values))
    elif mode == "min":
        index = int(np.argmin(dense_values))
    elif mode == "absolute":
        index = int(np.argmax(np.abs(dense_values)))
    else:
        raise ValueError("mode must be max, min, or absolute")
    return DensePeakResult(
        value=float(dense_values[index]),
        phase=float(dense_phases[index]),
    )
