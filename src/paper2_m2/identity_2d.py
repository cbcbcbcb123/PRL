"""True-2D, affine-periodic M2A identity-conversion mechanics.

The endocardium is a discrete chain, the cardiac-jelly/ECM is a plane-strain
standard-linear-solid continuum, and the myocardium is switched between a
central-force preferred-length network (DCM) and a plane-strain active-strain
continuum (FEM).  Every other block, port, interface and sampling convention is
shared.  FEniCSx/UFL assembles all continuum stiffness and drag operators.
"""
...
class ModelSystem:
    ...
    spatial_label: str
    passive_dcm_scale: float
    active_dcm_scale: float
    active_profile: str
...
def _dcm_network_full(
    ...
    thickness: float,
    spatial_profile: str,
) -> tuple[SparseMatrix, FloatArray, float]:
    coordinate_to_node = {
        (round(float(x), 13), round(float(y), 13)): node
        for node, (x, y) in enumerate(layer.dof_coordinates)
    }
    x_axis = np.linspace(-0.5 * length, 0.5 * length, nx + 1)
    y_axis = np.linspace(0.0, thickness, ny + 1)

    def node(ix: int, iy: int) -> int:
        return coordinate_to_node[
            (round(float(x_axis[ix]), 13), round(float(y_axis[iy]), 13))
        ]
    ...
    for iy in range(ny + 1):
        boundary_weight = 0.5 if iy in (0, ny) else 1.0
        for ix in range(nx):
            edges.append(
                (
                    node(ix, iy),
                    node(ix + 1, iy),
                    boundary_weight * dy / dx,
                )
            )
    for iy in range(ny):
        for ix in range(nx):
            edges.append((node(ix, iy), node(ix, iy + 1), dx / dy))
    diagonal_weight = 0.5 * dx * dy / (dx * dx + dy * dy)
    for iy in range(ny):
        for ix in range(nx):
            edges.append((node(ix, iy), node(ix + 1, iy + 1), diagonal_weight))
            edges.append((node(ix + 1, iy), node(ix, iy + 1), diagonal_weight))

    rows: list[int] = []
    columns: list[int] = []
    values: list[float] = []
    active = np.zeros(2 * len(layer.dof_coordinates), dtype=np.float64)
    active_scalar = 0.0
    for node_a, node_b, spring_weight in edges:
        edge = layer.dof_coordinates[node_b] - layer.dof_coordinates[node_a]
        edge_length = float(np.linalg.norm(edge))
        direction = edge / edge_length
        edge_vector = np.zeros(4, dtype=np.float64)
        edge_vector[:2] = -direction
        edge_vector[2:] = direction
        edge_dofs = np.asarray(
            (2 * node_a, 2 * node_a + 1, 2 * node_b, 2 * node_b + 1),
            dtype=np.int64,
        )
        for local_row, row in enumerate(edge_dofs):
            for local_column, column in enumerate(edge_dofs):
                rows.append(int(row))
                columns.append(int(column))
                values.append(
                    spring_weight
                    * edge_vector[local_row]
                    * edge_vector[local_column]
                )
        midpoint_x = float(
            0.5
            * (
                layer.dof_coordinates[node_a, 0]
                + layer.dof_coordinates[node_b, 0]
            )
        )
        profile_value = _activation_profile_value(
            midpoint_x, length, spatial_profile, FROZEN_CONFIG
        )
        preferred_increment = (
            edge_length * direction[0] ** 2 * profile_value
        )
        active[edge_dofs] += spring_weight * edge_vector * preferred_increment
        active_scalar += spring_weight * preferred_increment**2
    matrix = sparse.coo_matrix(
        (values, (rows, columns)),
        shape=(2 * len(layer.dof_coordinates), 2 * len(layer.dof_coordinates)),
    ).tocsr()
    return matrix, active, float(active_scalar)
...
def build_system(
    *,
    ...
    passive_dcm_scale: float = 1.0,
    active_dcm_scale: float = 1.0,
    active_profile: Literal["uniform", "S1"] = "uniform",
    config: M2AConfig = FROZEN_CONFIG,
) -> ModelSystem:
    config.checked()
    ...
    return build_system_for_level(
        representation=representation,
        level=level,
        passive_dcm_scale=passive_dcm_scale,
        active_dcm_scale=active_dcm_scale,
...
def build_system_for_level(
    *,
    ...
    """Build the unchanged M2A model on an explicitly authorized mesh level.

    Production callers use :func:`build_system`.  The explicit-level entry is
    reserved for bounded spatial diagnostics such as the approved S3 probe;
    it does not mutate the frozen production ladder or configuration digest.
    """
    ...
    if representation == "FEM":
        ...
        raw_matrix, raw_active, raw_scalar = _dcm_network_full(
            myocardium,
            level.nx,
            level.ny_per_layer,
            config.length,
            config.myocardium_thickness,
            active_profile,
        )
        myo_full = (passive_dcm_scale * raw_matrix).tocsr()
        active_full = (
            passive_dcm_scale * active_dcm_scale * raw_active
        )
        active_scalar = (
            passive_dcm_scale * active_dcm_scale**2 * raw_scalar
        )
        ...
        profile_average = float(
            np.mean(
                [
                    _activation_profile_value(x, config.length, active_profile, config)
                    for x in sample_x[:-1]
                ]
            )
        )
        manufactured_error = 0.0
    else:
    ...
    return ModelSystem(
        ...
        passive_dcm_scale=float(passive_dcm_scale),
        active_dcm_scale=float(active_dcm_scale),
        active_profile=active_profile,
...
def passive_calibration(
    *,
    config: M2AConfig = FROZEN_CONFIG,
) -> dict[str, float | str]:
    level = config.calibration_spatial_level
    fem_system = build_system(
        representation="FEM", spatial_label=level, config=config
    )
    dcm_raw = build_system(
        representation="DCM", spatial_label=level, config=config
    )
    scale = fem_system.passive_tangent / dcm_raw.passive_tangent
    return {
        "calibration_case": "ID-P1",
        "spatial_level": level,
        "fem_passive_tangent": fem_system.passive_tangent,
        "dcm_raw_passive_tangent": dcm_raw.passive_tangent,
        "dcm_passive_scale": float(scale),
        "relative_tangent_mismatch_after_calibration": float(
            abs(scale * dcm_raw.passive_tangent - fem_system.passive_tangent)
            / max(abs(fem_system.passive_tangent), 1.0e-30)
        ),
    }
...
def _case_components(
    ...
    active_profile = "S1" if case_id == "ID-S1" else "uniform"
    if case_id in {"ID-A1", "ID-A2", "ID-C0", "ID-CQ", "ID-S1"}:
    ...
    if case_id in {"ID-LN", "ID-C0", "ID-CQ"}:
...
def simulate_endpoint(
    *,
    ...
    case_id: str,
    steps_per_cycle: int,
    tolerance_label: ToleranceLabel,
) -> Endpoint:
    ...
    tolerance: SolverToleranceProfile = system.config.tolerance(tolerance_label)
    if case_id == "ID-P1":
    ...
    free_shortening = (
        (system.active_dcm_scale if system.representation == "DCM" else 1.0)
        * system.active_profile_average
        * activation
    )
    ...
    interface_magnitude = np.linalg.norm(interface_vectors, axis=1)
    ...
    summary = {
        ...
        "passive_dcm_scale": system.passive_dcm_scale,
        "active_dcm_scale": system.active_dcm_scale,
        ...
        "solver": {
            ...
            "acceptance_tolerance": tolerance.newton_fatol,
            **residuals,
            "maximum_relative_residual": maximum_solver_residual,
            "pass": bool(maximum_solver_residual <= tolerance.newton_fatol),
        },
        "ledger": {
            key: value
...
def active_work_calibration(
    *,
    passive_dcm_scale: float,
    config: M2AConfig = FROZEN_CONFIG,
) -> dict[str, float | str]:
    common = {
        "spatial_label": config.calibration_spatial_level,
        "steps_per_cycle": config.calibration_time_steps,
        "tolerance_label": config.calibration_tolerance,
    }
    fem = simulate_endpoint(
        system=build_system(
            representation="FEM",
            spatial_label=config.calibration_spatial_level,
            config=config,
    ...
    dcm_unit = simulate_endpoint(
        system=build_system(
            representation="DCM",
            spatial_label=config.calibration_spatial_level,
            passive_dcm_scale=passive_dcm_scale,
            active_dcm_scale=1.0,
            config=config,
        ),
        case_id="ID-A1",
        steps_per_cycle=config.calibration_time_steps,
    ...
    fem_work = float(fem.summary["ledger"]["total_active_work"])
    dcm_unit_work = float(dcm_unit.summary["ledger"]["total_active_work"])
    if fem_work <= 0.0 or dcm_unit_work <= 0.0:
        raise RuntimeError(
            f"active calibration requires positive work, got FEM={fem_work}, DCM={dcm_unit_work}"
        )
    active_scale = math.sqrt(fem_work / dcm_unit_work)
    return {
        "calibration_case": "ID-A1",
        **common,
        "fem_total_active_work": fem_work,
        "dcm_unit_total_active_work": dcm_unit_work,
        "dcm_active_scale": float(active_scale),
        "predicted_relative_work_mismatch_after_calibration": float(
            abs(active_scale**2 * dcm_unit_work - fem_work) / fem_work
        ),
    }


def build_calibration(config: M2AConfig = FROZEN_CONFIG) -> dict[str, Any]:
    passive = passive_calibration(config=config)
    active = active_work_calibration(
        passive_dcm_scale=float(passive["dcm_passive_scale"]),
        config=config,
    )
    return {
        "schema": "paper2_m2a_two_quantity_calibration_v01",
        "config_digest": config.digest(),
        "passive": passive,
        "active": active,
        "held_out_quantities_used": [],
    }
