"""Gate A M1 v03 solver with direct physical-gate candidate acceptance.

The mechanics, time discretization, optimizer configuration, and numerical
thresholds are imported from the frozen v01 implementation.  The only method
change is where a state may satisfy the physical equilibrium gate: every
finite objective/gradient evaluation is eligible, not only callback iterates.
"""
...
class GateAV03Run:
    ...
    step_audits: tuple[GateAV03StepAudit, ...]
    failure: GateAV03FailureSnapshot | None

    def summary(self) -> dict[str, Any]:
        """Combine frozen trajectory metrics with v03 method diagnostics."""
        counts: dict[str, int] = {}
...
@dataclass(frozen=True)
class GateAV03FailureSnapshot:
    """First rejected v03 candidate, kept outside the accepted trajectory."""

    reason: str
    audit: GateAV03StepAudit
    ...
    *,
    ...
    if initial_residual <= PROJECTED_RESIDUAL_LIMIT:
        if not _geometry_passes(initial_geometry):
            rejected_audit = GateAV03StepAudit(
...
def run_gate_a_v03_trajectory(
    case_id: str,
    *,
    dt: float,
    duration: float = 5.0,
) -> GateAV03Run:
    """Run one M1 v03 A0/A1 trajectory without retries."""
    ...
    failure: GateAV03FailureSnapshot | None = None
    for index in range(1, len(time)):
        ...
        if not step.accepted:
            if step.failure_reason is None:
                raise RuntimeError("rejected v03 step lacks a failure reason")
            failure = GateAV03FailureSnapshot(
...
def run_gate_a_v03_suite() -> tuple[
    dict[str, GateAV03Run],
    dict[str, Any],
]:
    """Run the complete registered M1 v03 suite and frozen acceptance."""
    configurations = (
    ...
    runs: dict[str, GateAV03Run] = {}
    for run_id, case_id, dt in configurations:
        run = run_gate_a_v03_trajectory(case_id, dt=dt)
        runs[run_id] = run
