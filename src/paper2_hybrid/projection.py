"""Conservative common-segment projection for active interface observables."""
...
def project_piecewise_linear_to_common_segments(
    ...
    """Return conservative P0 averages on a nested common grid."""
...
def common_segment_space_time_l2(
    ...
    return float(np.sqrt(time_step * np.sum(widths[:, None, None] * values * values)))


def constant_traction_projection_manufactured_check(
    *, native_segments: int, common_segments: int
) -> dict[str, float | bool | int]:
    if native_segments % common_segments != 0:
        raise ValueError("native segmentation must refine common segmentation")
    ...
    native_work = float(
        piecewise_linear_integral(native_x, np.sum(traction * displacement_increment, axis=1))
    )
    projected_work = float(
        common_segment_integral(
            common_x, np.sum(projected_traction * projected_increment, axis=1)
        )
    )
    force_error = float(np.linalg.norm(projected_force - native_force))
    constant_error = float(np.max(np.abs(projected_traction - traction_vector[None, :])))
