...
int main(int argc, char** argv) {
    try {
        if(argc < 4 || argc > 6) throw std::runtime_error("usage: probe mesh_input output_directory type_id [quadrature [adhesion_scale]]");
        ...
        if(argc>=5 && !quadrature)throw std::runtime_error("unknown contact strategy");
        if(quadrature)audit=model.run_surface_quadrature(cells,.20,argc==6?std::stod(argv[5]):1.0);
