...
int main(int argc,char** argv){
    ...
    try {
        ...
        if(!input || (count!=2) || clamped)throw std::runtime_error("invalid cell count/boundary combination");
        ...
        faces.flush();
        std::ofstream safety(output/"separation.csv");
        safety<<"step,coordinate,intercell_distance,nonincident_distance,min_height,increment\n";
        const auto assemble=[&](){
            ...
            const auto contact_audit=contact.run_surface_quadrature(cells,.10,1.0,.14);result.contact_energy=contact_audit.energy;result.support=contact_audit.support_area;
            for(unsigned cid=0;cid<count;++cid){
        ...
        for(unsigned step=1;coordinate<end_coordinate-1e-12;++step){
            ...
            const double increment=prl::contact_safety::safe_increment(separation,assembly.max_speed,requested);
