...
int main(int argc, char** argv) {
    try {
        ...
        if(count != 2 && count != 4 && count != 16) throw std::runtime_error("2/4/16 cells required");
        ...
        const double guard_seconds=std::chrono::duration<double>(std::chrono::steady_clock::now()-guard_start).count();
        if(geometry_only){
        ...
        const auto contact_start=std::chrono::steady_clock::now();
        ...
        const double contact_seconds=std::chrono::duration<double>(std::chrono::steady_clock::now()-contact_start).count();
        ...
        if(quadrature){
            ...
            metadata<<std::setprecision(17)<<"{\"contact_energy\":"<<audit.energy
                ...
                <<",\"active_samples\":"<<audit.active_samples<<",\"guard_seconds\":"<<guard_seconds<<",\"contact_seconds\":"<<contact_seconds<<"}\n";
        }
