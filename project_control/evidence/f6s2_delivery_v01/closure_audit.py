"""Verify exact delivery, navigation, preservation and Git scope; no science or deletion."""
import json
...
allowed={'START_HERE.md','project_control/CURRENT_STATUS.md','memory/project_cockpit/status.json',
         'memory/project_cockpit/task_log.jsonl','project_control/evidence/f6s2_delivery_v01/closure_audit.py',
         'project_control/evidence/f6s2_delivery_v01/closure_audit.json',
         'project_control/evidence/f6s2_delivery_v01/navigation_validation.json'}
def git(*args):
...
delivery_sources=root/'sources_at_delivery'
source_paths=[p for p in delivery_sources.rglob('*') if p.is_file()]
storage=result_admission(workspace,0)
checks={
    'manifest_identity':digest(root/'manifest.json')=='991710ecfb62d72bca971f8254199f119518d8fe199312e1bcea7949645d3e42',
    ...
    'delivery_sources_match':all(digest(p)==digest(workspace/p.relative_to(delivery_sources)) for p in source_paths),
...
report={'status':'passed' if all(checks.values()) else 'failed','checks':checks,
    ...
    'scientific_status':'failed','interface_fix_native_reexecution':'not_run','new_scientific_solves':0,
