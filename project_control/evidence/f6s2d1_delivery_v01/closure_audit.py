"""Final hash/link/Git-scope audit; no science or deletion."""
import json
...
allowed={'START_HERE.md','project_control/CURRENT_STATUS.md','memory/project_cockpit/status.json',
    'memory/project_cockpit/task_log.jsonl','project_control/evidence/f6s2d1_delivery_v01/closure_audit.py',
    'project_control/evidence/f6s2d1_delivery_v01/closure_audit.json',
    'project_control/evidence/f6s2d1_delivery_v01/navigation_validation.json'}
def git(*args):
...
checks={
    'manifest_identity':digest(root/'manifest.json')=='7bd712585b82c5ca4689f56292536a918b060ace2acf9051790ecba9a790e0f4',
    ...
    'strict_navigation':navigation['status']=='passed','git_exact_scope':not paths-protected-allowed,
    'original_dirty_set_present':protected<=paths,'branch_main':git('branch','--show-current').strip()=='main',
...
report={'status':'passed' if all(checks.values()) else 'failed','checks':checks,
    ...
    'scientific_status':'failed','fine_zero':'passed','fine_pressure':'failed','new_scientific_solves':0,
