...
try:
    import resource
except ModuleNotFoundError:  # pragma: no cover - Windows-only unit-test import
...
SCHEMA = "paper2_lineage_odd_mode_gate_v02"
...
RUNNER_PATH = "scripts/run_paper2_lineage_odd_mode_gate_v02.py"
SOURCE_HASHES = {
    ...
    "project_control/CURRENT_STATUS.md": (
        "de7f79c28b2f4d3f50e10db96bfb153f890e10201927f5ecaee029d97f16e06e"
    ),
...
SINGLE_CALL_BUDGET_SECONDS = 30.0
...
def _json_safe(value: Any, location: str = "$") -> Any:
    ...
    if isinstance(value, (float, np.floating)):
        ...
        if not math.isfinite(number):
            raise ValueError(
                f"non-finite floating-point value in JSON payload at {location}"
            )
        return number
    if isinstance(value, np.integer):
        return int(value)
    if isinstance(value, complex):
        return _json_safe(_complex_record(value), location)
    if isinstance(value, np.ndarray):
        return _json_safe(value.tolist(), location)
    if isinstance(value, Path):
        return value.as_posix()
    if isinstance(value, dict):
        return {
            str(key): _json_safe(item, f"{location}.{key}")
...
def _replace_nonfinite_with_markers(
    value: Any, location: str = "$", found: list[dict[str, str]] | None = None
) -> tuple[Any, list[dict[str, str]]]:
    records = [] if found is None else found
    if isinstance(value, (float, np.floating)) and not math.isfinite(float(value)):
        marker = repr(float(value))
        records.append({"location": location, "value": marker})
        return {"nonfinite_value": marker}, records
    if isinstance(value, complex):
        return _replace_nonfinite_with_markers(
            _complex_record(value), location, records
        )
    if isinstance(value, np.ndarray):
        return _replace_nonfinite_with_markers(value.tolist(), location, records)
    if isinstance(value, dict):
        converted: dict[str, Any] = {}
        for key, item in value.items():
            converted[str(key)], records = _replace_nonfinite_with_markers(
                item, f"{location}.{key}", records
            )
        return converted, records
    if isinstance(value, (list, tuple)):
        converted_list = []
        for index, item in enumerate(value):
            converted_item, records = _replace_nonfinite_with_markers(
                item, f"{location}[{index}]", records
            )
            converted_list.append(converted_item)
        return converted_list, records
...
def _maximum_absolute(interval: tuple[float, float]) -> float:
    return max(abs(interval[0]), abs(interval[1]))


def _depth_kernel_mixture_certificate(
    bands: list[dict[str, Any]],
) -> dict[str, Any]:
    """Conservative certificate for every convex mixture R[w]=sum_b w_b R_b.

    Per-band nonzero determinants do not imply a nonzero determinant after
    mixing.  This certificate uses the sufficient bound
    A_min*|g|_min > |a|_max*|B|_max when A and g keep a common sign.
    Failure of the sufficient bound is intentionally inconclusive.
    """

    a_intervals = [
        tuple(band["quantities"]["A"]["empirical_interval"]) for band in bands
    ]
    b_intervals = [
        tuple(band["quantities"]["B"]["empirical_interval"]) for band in bands
    ]
    little_a_intervals = [
        tuple(band["quantities"]["a"]["empirical_interval"]) for band in bands
    ]
    ...
    a_minimum = min(interval[0] for interval in a_intervals)
    g_all_positive = all(interval[0] > 0.0 for interval in g_intervals)
    g_all_negative = all(interval[1] < 0.0 for interval in g_intervals)
    if g_all_positive:
        g_minimum_absolute = min(interval[0] for interval in g_intervals)
        common_g_sign = "positive"
    elif g_all_negative:
        g_minimum_absolute = min(-interval[1] for interval in g_intervals)
        common_g_sign = "negative"
    else:
        g_minimum_absolute = 0.0
        common_g_sign = "mixed_or_unresolved"
    little_a_maximum_absolute = max(
        _maximum_absolute(interval) for interval in little_a_intervals
    )
    b_maximum_absolute = max(
        _maximum_absolute(interval) for interval in b_intervals
    )
    positive_term_lower = a_minimum * g_minimum_absolute
    cross_term_upper = little_a_maximum_absolute * b_maximum_absolute
    margin = positive_term_lower - cross_term_upper
    assumptions_pass = bool(
        a_minimum > 0.0 and (g_all_positive or g_all_negative)
    )
    certificate_pass = bool(assumptions_pass and margin > 0.0)
    return {
        "kernel_class": (
            "nonnegative probability weights constant within each of eight bands"
        ),
        "formula": "R[w]=sum_b w_b R_b; det=A*g-a*B",
        "method": "conservative_sufficient_bound",
        "A_minimum": a_minimum,
        "g_common_sign": common_g_sign,
        "g_minimum_absolute": g_minimum_absolute,
        "a_maximum_absolute": little_a_maximum_absolute,
        "B_maximum_absolute": b_maximum_absolute,
        "positive_term_lower": positive_term_lower,
        "cross_term_upper": cross_term_upper,
        "determinant_margin_lower": margin,
        "assumptions_pass": assumptions_pass,
        "certificate_pass": certificate_pass,
        "failure_is_not_no_go": True,
...
class SolveLedger:
    ...
    def _record_call(self, record: dict[str, Any], elapsed: float) -> None:
        record["seconds"] = elapsed
        record["single_call_budget_seconds"] = SINGLE_CALL_BUDGET_SECONDS
        record["single_call_pass"] = elapsed <= SINGLE_CALL_BUDGET_SECONDS
        self.records.append(record)
        if elapsed > SINGLE_CALL_BUDGET_SECONDS:
            raise TimeoutError(
                f"single linear-algebra call exceeded {SINGLE_CALL_BUDGET_SECONDS}s "
                f"at {record['label']}"
            )
    ...
    def factor(self, matrix: sparse.spmatrix, label: str) -> Any:
        ...
        self._record_call({"kind": "factorization", "label": label}, elapsed)
...
def _paired_analysis(discretizations: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    mixture_certificate = _depth_kernel_mixture_certificate(band_records)
    return {
        ...
        "bands": band_records,
        "depth_kernel_mixture_certificate": mixture_certificate,
    }
...
def _classify(geometry_pass: bool, paired: dict[str, Any]) -> dict[str, Any]:
    ...
    all_rank_open = all(rank_open_by_band)
    all_g_within_near_zero_scale = all(
        item[0] >= -ROBUST_NO_GO_TOLERANCE
        and item[1] <= ROBUST_NO_GO_TOLERANCE
        for item in g_intervals
    )
    mixture_certificate = paired["depth_kernel_mixture_certificate"]
    if all_g_within_near_zero_scale and all_rank_open:
        return {
            "status": "RESOLVED_SMALL_GAIN",
            "bounded_claim": (
                "rank-opening is numerically resolved but every g interval lies within "
                "the preregistered small-gain reference scale; feedback is not released"
            ),
            "rank_open_by_band": rank_open_by_band,
            "depth_kernel_mixture_certificate": mixture_certificate,
        }
    if all_g_within_near_zero_scale:
        return {
    ...
    if (
        a_positive
        and b_and_a_zero
        and (g_positive or g_negative)
        and all_rank_open
        and mixture_certificate["certificate_pass"]
    ):
        return {
            "status": "ROBUST_GO_LINEAGE_ODD_MODE",
            "bounded_claim": (
                "rank-opening, common g sign and the conservative full R[w] determinant "
                "certificate pass only within the two-grid empirical envelope and "
                "piecewise-constant nonnegative depth-kernel class"
            ),
            "rank_open_by_band": rank_open_by_band,
            "depth_kernel_mixture_certificate": mixture_certificate,
        }
    signs = [1 if item[0] > 0.0 else -1 if item[1] < 0.0 else 0 for item in g_intervals]
    if all_rank_open and 1 in signs and -1 in signs:
        return {
            "status": "CONDITIONAL_GO_NEEDS_DEPTH_EXPERIMENT",
            "bounded_claim": (
                "complete per-band rank-opening is resolved but the physical g sign "
    ...
    return {
        "status": "NOT_RESOLVED",
        "bounded_claim": (
            "the preregistered empirical gates or the conservative full-kernel "
            "determinant certificate do not support a signed conclusion"
        ),
        "rank_open_by_band": rank_open_by_band,
        "depth_kernel_mixture_certificate": mixture_certificate,
    }
...
def run(output: Path, base_commit: str, approved_runner_sha256: str) -> int:
    ...
    summary: dict[str, Any] = {
        ...
        "frozen_gates": {
            ...
            "single_linear_algebra_call_seconds": SINGLE_CALL_BUDGET_SECONDS,
    ...
    summary["execution"] = {
        ...
        "runtime": {
            ...
            "external_process_watchdog": os.environ.get(
                "PAPER2_EXTERNAL_WATCHDOG"
            ),
        },
    }
    return_code = 1
    try:
        ...
        if os.environ.get("PAPER2_EXTERNAL_WATCHDOG") != "host-60s-v01":
            raise RuntimeError("independent host process watchdog is not attested")
        ...
        if not geometry_pass:
            ...
            summary["depth_kernel_convex_hull"] = {
                ...
                "not_a_continuous_depth_limit": True,
                "full_R_mixture_certificate": paired[
                    "depth_kernel_mixture_certificate"
                ],
            }
        ...
        if resource is None:
            raise RuntimeError("Linux resource accounting is unavailable")
        ...
        try:
            ...
            marked_summary, nonfinite_records = _replace_nonfinite_with_markers(summary)
            marked_summary["status"] = "FAILED_CLOSED"
            marked_summary.setdefault("execution", {})["status"] = "FAIL"
            marked_summary["execution"]["serialization_error"] = str(
                serialization_error
            )
            marked_summary["execution"]["nonfinite_records"] = nonfinite_records
            marked_summary["scientific_interpretation"] = {
                "status": "NOT_EVALUABLE_DUE_TO_NONFINITE_OUTPUT",
                "not_a_scientific_negative_result": True,
            }
            safe_summary = _json_safe(marked_summary)
