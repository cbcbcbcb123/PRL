"""Explicit-exec cache retry adapter for the frozen static-mode screen v01.

The byte-locked v01 entry retains every scientific matrix, right-hand side,
observable, threshold, and output rule.  This adapter only validates the two
preserved launch failures, records v03 provenance, and requires the approved
executable /root/.cache tmpfs before delegating directly to v01.
"""
...
SCHEMA = "paper2_static_mode_screen_v03"
ACTUAL_LAUNCH_CHECKOUT = "476685b12e17a100abee1d571f8efd4f0e3db886"
CURRENT_STATUS_RELATIVE = Path("project_control/CURRENT_STATUS.md")
CURRENT_STATUS_SHA256 = "909c2c49fa161b5ba0f2a6b41e90329db72b9bedbae6aa073f5babff6cfebc55"
...
V02_ADAPTER_RELATIVE = Path("scripts/run_paper2_static_mode_screen_v02.py")
V02_ADAPTER_SHA256 = "cfaa421e60714c609dd58cb76582c032aff73a8d3e3ac2ae5b8467d410846435"
...
V02_SUMMARY_RELATIVE = Path("results/paper2_static_mode_screen/v02_20260905/summary.json")
V02_SUMMARY_SHA256 = "093ea5739285df4f66f0fc90743334a43c2c534f4b7f46bb4d79b038bd91f375"
V02_MANIFEST_RELATIVE = Path("results/paper2_static_mode_screen/v02_20260905/manifest.json")
V02_MANIFEST_SHA256 = "d2920a46f3c9a16314b9e99ff0aa9dbb36ca3b245c03ee2ffb87f3d216bea953"
RESULT_RELATIVE = Path("results/paper2_static_mode_screen/v03_20260905")
CONTAINER_NAME = "prl-paper2-static-mode-screen-v03-20260905"
V01_CHARGE_SECONDS = 0.05188476701732725
V02_NUMERICAL_CHARGE_SECONDS = 0.0
V02_CONTAINER_WALL_SECONDS = 1.041608602
V03_REMAINING_BUDGET_SECONDS = 299.9481152329827
V03_START_CUMULATIVE_SECONDS = 161.92011524902773
...
def _verify_retry_inputs(repo_root: Path) -> None:
    expected = {
        ...
        V02_ADAPTER_RELATIVE: V02_ADAPTER_SHA256,
        V01_SUMMARY_RELATIVE: V01_SUMMARY_SHA256,
        V01_MANIFEST_RELATIVE: V01_MANIFEST_SHA256,
        V02_SUMMARY_RELATIVE: V02_SUMMARY_SHA256,
        V02_MANIFEST_RELATIVE: V02_MANIFEST_SHA256,
    }
    for relative_path, expected_hash in expected.items():
        actual_hash = _sha256(repo_root / relative_path)
        if actual_hash != expected_hash:
            raise RuntimeError(
                f"v03 input hash mismatch: {relative_path.as_posix()} "
                f"expected={expected_hash} actual={actual_hash}"
            )
    for summary_relative, expected_failure in (
        (V01_SUMMARY_RELATIVE, "OSError"),
        (V02_SUMMARY_RELATIVE, "RuntimeError"),
    ):
        with (repo_root / summary_relative).open("r", encoding="utf-8") as handle:
        ...
        if not (
            ...
            and failed_summary["failure"]["type"] == expected_failure
        ):
            raise RuntimeError(f"preserved failure classification drift: {summary_relative}")
...
def _root_cache_mount() -> dict[str, Any]:
    ...
    if "noexec" in options:
        raise RuntimeError("/root/.cache tmpfs remains noexec after explicit-exec repair")
    return {
...
def _retry_metadata(repo_root: Path, mount_record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "authorization": {
            ...
            "scope": "one explicit-exec cache repair; no v04 on failure",
        },
        "version_identity": {
            "scientific_preregistration_and_production_baseline": core.EXPECTED_CODE_VERSION,
            "actual_checkout_at_v03_launch": ACTUAL_LAUNCH_CHECKOUT,
            "v01_scientific_core": {
                "path": CORE_RELATIVE.as_posix(),
                "sha256": CORE_SHA256,
                "unchanged_for_v02_and_v03": True,
                "not_claimed_to_exist_in_frozen_production_baseline": True,
            },
            "v02_operational_adapter": {
                "path": V02_ADAPTER_RELATIVE.as_posix(),
                "sha256": V02_ADAPTER_SHA256,
            },
            "v03_operational_adapter": {
                "path": adapter_relative.as_posix(),
                "sha256": _sha256(Path(__file__).resolve()),
                "imports_v01_directly_not_v02": True,
        ...
        "preserved_failed_attempts": {
            "v01": {
                ...
                "numerical_charge_seconds": V01_CHARGE_SECONDS,
                "summary_sha256": V01_SUMMARY_SHA256,
                "manifest_sha256": V01_MANIFEST_SHA256,
            },
            "v02": {
                "classification": "ROOT_CACHE_TMPFS_NOEXEC_OBSERVED_NOT_SCIENTIFIC_NEGATIVE",
                "completed_assemblies": 0,
                "completed_static_rhs": 0,
                "numerical_charge_seconds": V02_NUMERICAL_CHARGE_SECONDS,
                "container_wall_seconds_separate": V02_CONTAINER_WALL_SECONDS,
                "summary_sha256": V02_SUMMARY_SHA256,
                "manifest_sha256": V02_MANIFEST_SHA256,
            },
            "preserved_without_overwrite_or_container_removal": True,
        },
        "budget": {
            "original_static_numerical_budget_seconds": 300.0,
            "v01_charged_seconds": V01_CHARGE_SECONDS,
            "v02_numerical_charge_seconds": V02_NUMERICAL_CHARGE_SECONDS,
            "v03_remaining_budget_seconds": V03_REMAINING_BUDGET_SECONDS,
            "v03_start_cumulative_fem_and_failure_charge_seconds": V03_START_CUMULATIVE_SECONDS,
        },
        "only_operational_change": {
            "description": "explicit exec on the writable /root/.cache tmpfs",
            "docker_requested_options": "rw,exec,nosuid,nodev,size=536870912",
...
def _configure_core() -> None:
    ...
    core.NUMERICAL_BUDGET_SECONDS = V03_REMAINING_BUDGET_SECONDS
    core.OLD_FEM_AND_FAILURE_CHARGE_SECONDS = V03_START_CUMULATIVE_SECONDS
...
def main() -> int:
    ...
    if os.environ.get("PAPER2_STATIC_ACTUAL_CHECKOUT") != ACTUAL_LAUNCH_CHECKOUT:
        raise RuntimeError("v03 checkout environment mismatch")
