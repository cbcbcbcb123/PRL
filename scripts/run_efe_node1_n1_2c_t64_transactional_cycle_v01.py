...
def main() -> None:
    ...
    if output_path.exists():
        raise FileExistsError(f"T64 cycle output already exists: {output_path}")
    if not input_path.is_file():
        raise FileNotFoundError(f"T64 warm checkpoint is missing: {input_path}")
    if not (previous_cycle / "cycle_timeseries.csv").is_file():
        raise FileNotFoundError("accepted T32 comparison cycle is incomplete")
    ...
    try:
        ...
        failure = {
            "schema_version": "efe_node1_n1_2c_t64_transactional_failure_v01",
    ...
    if int(inner_summary.get("steps_per_cycle", -1)) != TARGET_STEPS:
        raise RuntimeError("inner transaction engine did not execute T64")
    ...
    if passed_transactions != 2 * TARGET_STEPS:
        raise RuntimeError("T64 accepted transaction count is incomplete")
    ...
    if not bool(inner_summary.get("all_worker_process_ids_unique")):
        raise RuntimeError("T64 transaction workers were not process isolated")

    report = {
        "schema_version": "efe_node1_n1_2c_t64_transactional_cycle_v01",
        "status": inner_summary["status"],
        "authorization": arguments.authorization,
        "input_checkpoint": str(input_path),
        "previous_t32_cycle_directory": str(previous_cycle),
        ...
        "evidence_boundary": (
            "This result tests within-level T64 cycle stability only. P5 formal "
            "three-level time adjudication, Figure 2, and T128 are not authorized."
        ),
    }
