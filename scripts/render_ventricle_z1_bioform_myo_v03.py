"""Render solver-derived figures and an offline evidence index for Z1-BIOFORM-MYO-R v03."""
...
FIGURE_STEMS = ["model_structure", "morphology_timepoints", "mechanical_fields", "repair_diagnostics", "validation_gates"]
...
def render_model_structure(output: Path) -> None:
    ...
    axis.set_ylim(0, 7)
    axis.axis("off")
    axis.text(0.3, 6.62, "SCHEMATIC — repaired free myocardial cell; not a solver result", fontsize=14, weight="bold")
    ...
    blocks = [
        ...
        (7.0, 3.10, "PASSIVE", "cortical tension + area elasticity\nvolume pressure; bending quarantined", "#dce9f7", "#3d6f9e"),
        (7.0, 1.25, "NUMERICAL", "total-velocity rigid gauge +\nadaptive overdamped substeps", "#e5ead8", "#6b7d36"),
    ]
    ...
    axis.text(11.65, 0.22, "Z1-BIOFORM-MYO-R v03", ha="right", color="#6b747d")
...
def render_repair_diagnostics(output: Path) -> None:
    v02_root = PROJECT_ROOT / "results" / "ventricle_z1" / "z1_bioform_myo_a_v02_20260913" / "raw"
    v03_root = output / "raw"
    old_dt040 = condition_metrics(v02_root / "FULL_M320_DT040")
    new_dt040 = condition_metrics(v03_root / "FULL_M320_DT040")
    figure, axes = plt.subplots(1, 3, figsize=(15.2, 4.5))

    for label, item, color_value in (
        ("v02 legacy bending", old_dt040, "#bf4b43"),
        ("v03 repaired", new_dt040, "#287f73"),
    ):
        coordinates = [snapshot["solver_coordinate"] for snapshot in item["snapshots"]]
        center_norms = [np.linalg.norm(snapshot["center"]) / EQUIVALENT_RADIUS for snapshot in item["snapshots"]]
        axes[0].plot(coordinates, center_norms, "o-", label=label, color=color_value)
    axes[0].set_yscale("symlog", linthresh=1.0e-5)
    axes[0].set_xlabel("algorithmic coordinate λ")
    axes[0].set_ylabel("centroid drift / equivalent radius")
    axes[0].set_title("Rigid-drift failure removed")
    axes[0].legend(frameon=False, fontsize=8)

    step_ids = ["FULL_M320_DT040", "FULL_M320_DT020", "FULL_M320_DT010"]
    step_values = [0.040, 0.020, 0.010]
    for root, label, linestyle, alpha in (
        (v02_root, "v02", "--", 0.62),
        (v03_root, "v03", "-", 1.0),
    ):
        items = [condition_metrics(root / condition_id)["final"] for condition_id in step_ids]
        axes[1].plot(step_values, [item["E_pq"] for item in items], "o" + linestyle, color="#c65449", alpha=alpha, label=f"{label} E_pq")
        axes[1].plot(step_values, [item["F_qr"] for item in items], "s" + linestyle, color="#477fae", alpha=alpha, label=f"{label} F_qr")
    axes[1].invert_xaxis()
    axes[1].set_xlabel("outer numerical step")
    axes[1].set_ylabel("final shape ratio")
    axes[1].set_title("Step-size branch recovery")
    axes[1].legend(frameon=False, fontsize=8, ncol=2)

    axes[2].axis("off")
    axes[2].text(0.02, 0.94, "Causal repair", fontsize=13, weight="bold", va="top")
    lines = [
        "Legacy bending: failed rigid invariance",
        "→ quarantined (k_b = 0 in this stage)",
        "",
        "Active tensor normal drive retained",
        "→ no target shape / no reference metric",
        "",
        "Total overdamped velocity",
        "→ remove 3 translations + 3 rotations",
        "→ cap displacement at 10% shortest edge",
        "",
        "Curvature is still measured and mapped.",
        "Biological validation remains blocked_data.",
    ]
    axes[2].text(0.02, 0.84, "\n".join(lines), fontsize=10.2, va="top", linespacing=1.35)
    figure.suptitle("Numerical repair evidence: v02 failure versus v03", fontsize=14, weight="bold")
    figure.tight_layout(rect=(0, 0.01, 1, 0.93))
    save_figure(figure, output, "repair_diagnostics")
...
<title>Z1-BIOFORM-MYO-R v03</title><style>
...
</style></head><body><main><section class="hero"><h1>Z1-BIOFORM-MYO-R v03</h1><p class="status">{status}</p>
<p>单个自由心肌细胞的合成内禀形态机制数值修复资格。未合格弯曲项已隔离；结果不代表生物学参数已标定，父 Z1 仍为 <code>blocked</code>。</p>
...
def main() -> int:
    ...
    render_repair_diagnostics(output)
