"""Run the repaired-launch v02 eight-endpoint transverse-notch test.
...
SCHEMA = "paper2_transverse_notch_v02"
CASE_SCHEMA = "paper2_transverse_notch_case_v02"
SUMMARY_SCHEMA = "paper2_transverse_notch_summary_v02"
EXPECTED_PROJECT_ROOT = Path("/workspace")
EXPECTED_OUTPUT_ROOT = Path("/output")
HOST_OUTPUT_RELATIVE = "results/paper2_transverse_notch/v02_20260905"
PREDICTION_COMMIT = "9842b3bfab7f11e947d0c355fa8b0202a368138f"
EXPECTED_RUNTIME_SOURCE_VERSION = "7cf40e211ccbd01b5baa60693834ea7907b89405"
...
FAILED_ATTEMPT_PATH = Path("results/paper2_transverse_notch/v01_20260905")
FAILED_ATTEMPT_SUMMARY_PATH = FAILED_ATTEMPT_PATH / "summary.json"
FAILED_ATTEMPT_SUMMARY_SHA256 = (
    "62049eaaf52c1a701c5b4bb9b539cfaaa56c23c9cfdfa9b294b25e7299453f3d"
)
EXPECTED_IMAGE_ID = (
    "sha256:2ae4bfbc0d9077268880faf04c72750528bee986c94ab223a2c159969bd56fa8"
)
EXPECTED_CONTAINER_NAME = "prl-paper2-transverse-notch-v02-20260905"
EXPECTED_PYTHONPATH = (
...
ORIGINAL_BATCH_WALL_BUDGET_SECONDS = 900.0
FAILED_STARTUP_BUDGET_DEBIT_SECONDS = 3.0
BATCH_WALL_BUDGET_SECONDS = 897.0
...
def _verify_frozen_inputs(repo_root: Path, prediction_commit: str) -> dict[str, Any]:
    if prediction_commit != PREDICTION_COMMIT:
        raise RuntimeError(
            f"prediction commit drift: {prediction_commit!r} != {PREDICTION_COMMIT!r}"
        )
    ...
    if prior_elapsed + FAILED_STARTUP_BUDGET_DEBIT_SECONDS >= PROJECT_WALL_BUDGET_SECONDS:
        raise RuntimeError("project cumulative wall budget was already exhausted")

    failed_attempt_hash = pilot._sha256(repo_root / FAILED_ATTEMPT_SUMMARY_PATH)
    if failed_attempt_hash != FAILED_ATTEMPT_SUMMARY_SHA256:
        raise RuntimeError(
            f"failed-attempt summary hash drift: {failed_attempt_hash} != "
            f"{FAILED_ATTEMPT_SUMMARY_SHA256}"
        )
    with (repo_root / FAILED_ATTEMPT_SUMMARY_PATH).open("r", encoding="utf-8") as handle:
        failed_attempt = json.load(handle)
    if (
        failed_attempt.get("status") != "ABORTED_FAIL_CLOSED"
        or failed_attempt.get("completed_case_count") != 0
        or failed_attempt.get("error_message") != "No module named 'dolfinx'"
    ):
        raise RuntimeError("failed-attempt evidence does not match the authorized retry")
    ...
    return {
        ...
        "failed_attempt": {
            "path": FAILED_ATTEMPT_PATH.as_posix(),
            "summary_path": FAILED_ATTEMPT_SUMMARY_PATH.as_posix(),
            "summary_sha256": failed_attempt_hash,
            "status": "ABORTED_FAIL_CLOSED",
            "completed_fem_endpoint_count": 0,
            "failure_stage": "dependency import before FEM",
            "failure_reason": "PYTHONPATH override made dolfinx unavailable",
            "budget_debit_seconds": FAILED_STARTUP_BUDGET_DEBIT_SECONDS,
        },
...
def run(
    ...
    if os.environ.get("PYTHONPATH") != EXPECTED_PYTHONPATH:
        raise RuntimeError(
            f"PYTHONPATH drift: {os.environ.get('PYTHONPATH')!r} != "
            f"{EXPECTED_PYTHONPATH!r}"
        )
    if runtime_source_version != EXPECTED_RUNTIME_SOURCE_VERSION:
        raise RuntimeError(
            f"runtime source version drift: {runtime_source_version!r} != "
            f"{EXPECTED_RUNTIME_SOURCE_VERSION!r}"
        )
    ...
    frozen_inputs = _verify_frozen_inputs(repo_root, prediction_commit)

    import paper2_hybrid
    import dolfinx
    import scipy

    paper2_hybrid_path = Path(paper2_hybrid.__file__).resolve()
    dolfinx_path = Path(dolfinx.__file__).resolve()
    expected_paper2_root = (repo_root / "src" / "paper2_hybrid").resolve()
    expected_dolfinx_root = Path(
        "/usr/local/dolfinx-real/lib/python3.12/dist-packages/dolfinx"
    ).resolve()
    if not str(paper2_hybrid_path).startswith(str(expected_paper2_root) + os.sep):
        raise RuntimeError(
            f"paper2_hybrid import path drift: {paper2_hybrid_path}"
        )
    ...
    manifest = {
        ...
        "runtime_source": {
            "git_version_at_launch": runtime_source_version,
            "distinct_from_prediction_commit": runtime_source_version != prediction_commit,
        },
        "runner": {
            "path": "scripts/run_paper2_transverse_notch_v02.py",
        ...
        "runtime": {
            ...
            "dependency_import_regression": {
                ...
                "paper2_hybrid_path": paper2_hybrid_path.as_posix(),
                "paper2_hybrid_expected_root": expected_paper2_root.as_posix(),
                "dolfinx_path": dolfinx_path.as_posix(),
                "dolfinx_expected_root": expected_dolfinx_root.as_posix(),
                "pass": True,
            },
        },
        "budgets": {
            "original_batch_wall_seconds": ORIGINAL_BATCH_WALL_BUDGET_SECONDS,
            "failed_startup_budget_debit_seconds": FAILED_STARTUP_BUDGET_DEBIT_SECONDS,
            "v02_available_wall_seconds": BATCH_WALL_BUDGET_SECONDS,
        ...
        "failed_attempt": frozen_inputs["failed_attempt"],
    ...
    for position, spec in enumerate(CASE_SPECS, start=1):
        ...
        project_elapsed_before = (
            frozen_inputs["prior_elapsed_seconds"]
            + FAILED_STARTUP_BUDGET_DEBIT_SECONDS
            + elapsed_before
        )
