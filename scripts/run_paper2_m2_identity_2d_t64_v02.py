"""Execute only the authorized M2A v02 T64 full numerical gate."""
...
from paper2_m2.identity_2d_v02 import (  # noqa: E402
...
from paper2_m2.protocol_v02 import (  # noqa: E402
    FROZEN_PROTOCOL_V02,
...
def _hash_paths(paths: tuple[Path, ...]) -> dict[str, str]:
    return {_relative_project_path(path): _sha256(path) for path in paths}
...
def _write_hash_ledger(output_dir: Path) -> Path:
    ...
    _write_json(
        ledger_path,
        {
            "schema": "paper2_m2a_v02_t64_hash_ledger_v01",
            "files": entries,
        },
    )
    return ledger_path
...
def _selected_npz_finite_audit(selected_dir: Path) -> dict[str, Any]:
    ...
    for path in sorted(selected_dir.glob("*.npz")):
        ...
        records[path.name] = {"arrays": arrays, "pass": all(v["finite"] for v in arrays.values())}
    expected = len(DYNAMIC_HOLDOUT_CASES) * len(FROZEN_PROTOCOL_V02.representations)
    return {
        "schema": "paper2_m2a_v02_t64_finite_value_audit_v01",
        ...
        "pass": all_finite and len(records) == expected,
    }
...
def _parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--supervisor-decision", type=Path, required=True)
    parser.add_argument("--autonomous-decision", type=Path, required=True)
    parser.add_argument("--base-contract-v01", type=Path, required=True)
    parser.add_argument("--v01-failure-record", type=Path, required=True)
    parser.add_argument("--s4-execution-record", type=Path, required=True)
...
def main() -> None:
    ...
    try:
        ...
        failure = {
            "schema": "paper2_m2a_v02_t64_failure_v01",
            "status": "FAIL_CLOSED",
            "stage": "source_calibration_lock",
            "reason": str(error),
    ...
    preflight_tests = {
        "schema": "paper2_m2a_v02_t64_preflight_tests_v01",
    ...
    frozen_hashes_before = _hash_paths(FROZEN_V01_PATHS)
    ...
    manifest = {
        "schema": "paper2_m2a_identity_2d_v02_t64_manifest_v01",
        "protocol": protocol.canonical_payload(),
        "protocol_digest": protocol.digest(),
        "source_calibration_sha256": _sha256(arguments.source_calibration),
        "frozen_v01_sha256_before": frozen_hashes_before,
        "implementation_sha256": _hash_paths(implementation_paths),
        "provenance": {
            "contract_v02_sha256": _sha256(arguments.contract_v02),
            "supervisor_decision_sha256": _sha256(arguments.supervisor_decision),
            "autonomous_decision_sha256": _sha256(arguments.autonomous_decision),
            "base_contract_v01_sha256": _sha256(arguments.base_contract_v01),
            "v01_failure_record_sha256": _sha256(arguments.v01_failure_record),
            "s4_execution_record_sha256": _sha256(arguments.s4_execution_record),
        },
        "runtime": {
    ...
    projection_checks = {
        "schema": "paper2_m2a_v02_t64_projection_preflight_v01",
    ...
    assembly_probe = {
        "schema": "paper2_m2a_v02_t64_S4_assembly_probe_v01",
        "representation": "FEM",
        "profile": "uniform",
        ...
        for representation in protocol.representations:
            for level in protocol.spatial_levels:
                ...
                for tolerance_label in protocol.tolerance_labels:
                    ...
                    if (
                        case_id in DYNAMIC_HOLDOUT_CASES
                        and level.label == "S4"
    ...
    frozen_hashes_after = _hash_paths(FROZEN_V01_PATHS)
    ...
    if (
        not all_structural_pass
        or not stage_gate["pass"]
        or not finite_audit["pass"]
        or not frozen_unchanged
        or not within_resource_budget
    ):
        failure_reasons = []
        if not all_structural_pass:
            failure_reasons.append("structural_gate")
        if not stage_gate["pass"]:
            failure_reasons.append("T64_numerical_gate")
        if not finite_audit["pass"]:
            failure_reasons.append("finite_value_audit")
        if not frozen_unchanged:
            failure_reasons.append("frozen_v01_hash_drift")
        if not within_resource_budget:
            failure_reasons.append("resource_budget")
        failure = {
            "schema": "paper2_m2a_v02_t64_failure_v01",
            "status": "FAIL_CLOSED_AT_SUPERVISOR_GATE",
            "stage": "T64_full_numerical_gate",
            "failure_reasons": failure_reasons,
            "numerical_failures": stage_gate["failures"],
            "completed_endpoints": endpoint_counter,
            "expected_endpoints": protocol.expected_endpoint_count,
            "frozen_v01_sha256_before": frozen_hashes_before,
            "frozen_v01_sha256_after": frozen_hashes_after,
            "resource": {
    ...
    pass_summary = {
        "schema": "paper2_m2a_v02_t64_pass_summary_v01",
        "status": "COMPLETE_AT_SUPERVISOR_GATE",
        "decision": "T64_NUMERICAL_PASS",
        "completed_endpoints": endpoint_counter,
        "expected_endpoints": protocol.expected_endpoint_count,
        ...
        "frozen_v01_sha256_before": frozen_hashes_before,
        "frozen_v01_sha256_after": frozen_hashes_after,
        "frozen_v01_unchanged": frozen_unchanged,
        "resource": {
            "elapsed_seconds": elapsed_seconds,
            "budget_seconds": protocol.runtime_budget_seconds,
            "peak_memory_gib": peak_memory_gib,
        ...
        "evidence_boundary": (
            "T64_NUMERICAL_PASS is a v02 numerical credibility result only. It is not "
            "a DCM-FEM identity decision, physiological validation, or authorization for T128."
        ),
    }
    _write_json(arguments.output_dir / "pass_summary.json", pass_summary)
    _write_hash_ledger(arguments.output_dir)
    print(json.dumps(pass_summary, indent=2, sort_keys=True, allow_nan=False), flush=True)
