...
DEFAULT_OUTPUT = (
    ROOT / "results/hybrid/prl_p6_t128_warm_start_v02_20260827"
)
AUTHORIZATION = (
    "project_control/"
...
def resample_cycle_csv(source: Path, target: Path) -> dict[str, object]:
    ...
    if not fields or validate_steps_per_cycle(source_steps) != 64:
        raise ValueError("T128 warm start requires one complete T64 source cycle")
...
def main() -> None:
    ...
    if output_path.exists():
        raise FileExistsError(f"T128 warm-start output already exists: {output_path}")
    ...
    if missing:
        raise FileNotFoundError(f"T64 source cycle is incomplete: {missing}")
    ...
    for name, refined in refined_arrays.items():
        ...
        if not exact:
            raise RuntimeError(f"T64 common phases changed for {name}")

    resampling_summary = {
        "schema_version": "prl_p6_t64_to_t128_resampling_v01",
        ...
        "passed": True,
        "evidence_boundary": (
            "Interpolated intermediate phases initialize the T128 frozen-geometry "
            "warm calculation only; they are not accepted T128 dynamic states."
        ),
    }
    ...
    if process.returncode != 0:
        failure = {
            "schema_version": "prl_p6_t128_warm_start_failure_v01",
        ...
        raise RuntimeError("T128 inner warm engine failed")
    ...
    if not bool(inner_summary.get("passed")):
        raise RuntimeError("T128 inner warm engine did not pass its gate")
    ...
    if root_commit["array_digest"] != checkpoint_array_digest(
        ...
        raise RuntimeError("T128 root warm checkpoint digest mismatch")

    report = {
        "schema_version": "prl_p6_t128_warm_start_v01",
        "status": "passed_t128_reperiodized_warm_start",
        "authorization": arguments.authorization,
        "source_cycle_directory": str(source_cycle),
        ...
        "passed": True,
        "evidence_boundary": (
            "This is a T128 frozen-geometry viscoelastic fixed-point warm start "
            "for the PRL theory-first baseline. It is not T128 cycle-stability "
            "or time-convergence evidence."
        ),
    }
