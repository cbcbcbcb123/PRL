"""Independent verifier for Z1-BIOFORM-MYO-R v03."""
...
def _center_over_radius(item: dict[str, Any]) -> float:
    return math.sqrt(sum(float(value) ** 2 for value in item["center"])) / EQUIVALENT_RADIUS
...
def main() -> int:
    ...
    if verification_dir.exists() or (output / "summary.json").exists():
        raise SystemExit("create-only verification output already exists")
    ledger = json.loads((output / "run_ledger.json").read_text(encoding="utf-8"))
    repair_selection = json.loads((output / "repair_selection.json").read_text(encoding="utf-8"))
    verification_dir.mkdir()
    ...
    if raw_complete:
        for condition_id in expected_ids:
            item = metrics[condition_id]
            kernel = item["kernel"]
            max_center = max(_center_over_radius(snapshot) for snapshot in item["snapshots"])
            gates.extend(
                [
                    gate(f"{condition_id}:finite_saved_states", item["all_saved_states_finite"], "true", item["all_saved_states_finite"], "common_numerical"),
                    gate(f"{condition_id}:sphere_topology", item["all_saved_states_sphere_topology"], "true", item["all_saved_states_sphere_topology"], "common_numerical"),
                    gate(f"{condition_id}:max_volume_error", kernel["max_volume_relative_error"], "<=0.02", kernel["max_volume_relative_error"] <= 0.02, "common_numerical"),
                    gate(f"{condition_id}:min_saved_angle_deg", item["minimum_saved_triangle_angle_deg"], ">=15", item["minimum_saved_triangle_angle_deg"] >= 15.0, "common_numerical"),
                    gate(f"{condition_id}:min_step_angle_deg", kernel["minimum_step_triangle_angle_deg"], ">=15", kernel["minimum_step_triangle_angle_deg"] >= 15.0, "common_numerical"),
                    gate(f"{condition_id}:velocity_translation_residual", kernel["maximum_relative_translation_residual"], "<=1e-12", kernel["maximum_relative_translation_residual"] <= 1.0e-12, "common_numerical"),
                    gate(f"{condition_id}:velocity_rotation_residual", kernel["maximum_relative_rotation_residual"], "<=1e-12", kernel["maximum_relative_rotation_residual"] <= 1.0e-12, "common_numerical"),
                    gate(f"{condition_id}:work_residual", kernel["max_work_dissipation_relative_residual"], "<=1e-12", kernel["max_work_dissipation_relative_residual"] <= 1.0e-12, "common_numerical"),
                    gate(f"{condition_id}:centroid_drift_over_radius", max_center, "<=0.005", max_center <= 0.005, "common_numerical"),
                    gate(f"{condition_id}:positive_substep", kernel["minimum_accepted_substep"], ">0", kernel["minimum_accepted_substep"] > 0.0, "common_numerical"),
                    gate(f"{condition_id}:bending_disabled", kernel["bending_modulus"], "==0", kernel["bending_modulus"] == 0.0, "common_numerical"),
                    gate(f"{condition_id}:target_geometry_disabled", kernel["target_geometry_used"], "false", kernel["target_geometry_used"] is False, "common_numerical"),
                    gate(f"{condition_id}:reference_shape_terms", [kernel["reference_edge_shape_terms"], kernel["reference_face_metric_terms"]], "[0,0]", kernel["reference_edge_shape_terms"] == 0 and kernel["reference_face_metric_terms"] == 0, "common_numerical"),
                    gate(f"{condition_id}:rigid_mode_policy", kernel["rigid_mode_policy"], "area_weighted_total_velocity_projection", kernel["rigid_mode_policy"] == "area_weighted_total_velocity_projection", "common_numerical"),
                ]
            )
    ...
    payload = {
        ...
        "biological_validation_status": "blocked_data",
        "parent_Z1_status": "blocked",
        "selected_stress_amplitude": repair_selection["stress_amplitude"],
        "source_hashes_match": source_hashes_match,
        "hash_checks": hash_checks,
        "gates": gates,
        "condition_metrics": metrics,
    ...
    with (verification_dir / "condition_metrics.csv").open("w", encoding="utf-8", newline="") as stream:
        writer = csv.writer(stream)
        writer.writerow(["condition", "E_pq", "F_qr", "max_volume_error", "min_saved_angle_deg", "min_step_angle_deg", "force_ratio", "final_faces", "centroid_over_radius"])
        for condition_id in expected_ids:
            if condition_id not in metrics:
                continue
            item = metrics[condition_id]
            writer.writerow(
                [
                    ...
                    item["kernel"]["max_volume_relative_error"],
                    item["minimum_saved_triangle_angle_deg"],
                    item["kernel"]["minimum_step_triangle_angle_deg"],
                    item["kernel"]["final_to_peak_free_force_ratio"],
                    item["final"]["face_count"],
                    _center_over_radius(item["final"]),
                ]
            )
    ...
    summary = {
        ...
        "formal_matrix_completed": raw_complete,
        "selected_stress_amplitude": repair_selection["stress_amplitude"],
        "bending_status": "disabled_pending_invariant_core_repair",
        "failed_gate_count": len(failed_gates),
        "failed_gates": failed_gates,
        "main_final": metrics.get("FULL_M320_DT020", {}).get("final"),
        "evidence": {
            ...
            "condition_metrics": "verification/condition_metrics.csv",
            "run_ledger": "run_ledger.json",
            "diagnosis": str(PROJECT_ROOT / "project_control" / "ventricle_bioform_myocardium_repair_diagnosis_v03.md"),
        },
    }
    write_json(output / "summary.json", summary)
    report_lines = [
        "# Z1-BIOFORM-MYO-R v03 独立核验报告",
        "",
        f"- 合成机制裁决：`{verdict}`",
        "- 生物学验证：`blocked_data`",
        "- 父 Z1：`blocked`（本阶段不提升父阶段）",
        f"- 正式矩阵：{'8/8 completed' if raw_complete else 'incomplete'}",
        f"- 失败门数：{len(failed_gates)}",
        "- 当前离散弯曲项：`disabled_pending_invariant_core_repair`",
    ]
    if metrics.get("FULL_M320_DT020"):
        main_final = metrics["FULL_M320_DT020"]["final"]
        report_lines.extend(
    ...
    if failed_gates:
        ...
        report_lines.append("- 无。全部冻结的数值、机制与离散门均通过。")
    report_lines.extend(
        [
            "",
            "## 主张边界",
            "",
            "该结果只裁决合成的单个自由心肌细胞内禀形态机制。弯曲项的关闭是对未通过刚体不变性实现的隔离，不代表真实细胞弯曲刚度为零。没有同期三维实验轴长数据，因此不得称为真实心肌形态已验证；也不得据此宣称心内膜、ECM 或三层组织完成。",
        ]
    )
    (output / "report.md").write_text("\n".join(report_lines) + "\n", encoding="utf-8")
    print(json.dumps(summary, ensure_ascii=False, indent=2))
