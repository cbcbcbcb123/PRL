...
2. **Hybrid X0–X1 implementation**：采用 PRL 自有 cell engine + 独立体积 ECM 的混合架构，不覆盖 Route H 历史结果。X0-A–E 已完成；X1-A–J 的逐级接口均在各自 coverage 内冻结。X1-K v01 cube 负阶结果保留为非光滑奇异族诊断；v02 的 `gamma*A` 方向导数通过，但四层 icosphere 瞬时速度因 coarse 规则正二十面体的对称超收敛而违反预冻结误差非增门禁，已停止为 `failed_instantaneous_smooth_surface_refinement`。smooth S1 与 R1/C1/F1 均未执行。
...
长期架构决策见 `docs/adr/0001-owned-cpp-core-python-research-layer.md`，X0-C/D 冻结接口见 `docs/hybrid_architecture_v02.md`，X0-E 自有仓库拓扑见 `docs/hybrid_architecture_v03.md`，X1-A–J 的递进冻结见 `docs/hybrid_architecture_v04.md` 至 `docs/hybrid_architecture_v13.md`；X1-K v01/v02 失败冻结见 `docs/hybrid_architecture_v14.md` 与 `docs/hybrid_architecture_v15.md`。外部科学评审后的硬约束见 `project_control/external_scientific_review_constraints_v01.md`，X1-K 合同见 `project_control/hybrid_x1_k_acceptance_contract_v01.md` 与 `project_control/hybrid_x1_k_acceptance_contract_v02.md`。当前不得继续 ECM/血流长耦合、参数标定或论文机制 claim，也不得把局部 gate 通过描述为完整短轨迹或稳定收缩轨迹已经验证。
